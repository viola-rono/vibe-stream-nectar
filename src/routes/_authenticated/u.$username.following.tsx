...
export const Route = createFileRoute("/_authenticated/u/$username/following")({
  head: ({ params }) => ({ meta: [{ title: `@${params.username} is following — Embr` }] }),
    ...
    <div className="min-h-dvh bg-background pb-28">
      <header className="brand-gradient text-white sticky top-0 z-30">
        <div className="mx-auto max-w-2xl px-3 py-3 flex items-center gap-2">
          ...
          <h1 className="font-bold text-base truncate flex-1">@{username} · Following</h1>
        </div>
      ...
      <div className="mx-auto max-w-2xl divide-y divide-border">
        ...
        {!isLoading && filtered.length === 0 && (
          <div className="p-10 text-center text-sm text-muted-foreground">Not following anyone yet</div>
        )}
